use std::{collections::HashMap, sync::Arc, thread::spawn, time::Duration};

use axum::{extract::{ws::{Message, WebSocket}, State, WebSocketUpgrade}, response::IntoResponse, routing::get, Router};
use futures::{lock::Mutex, StreamExt};
use serde_json::Value;
use tokio::{sync::broadcast,sync::mpsc, time::interval};
            ...
            Message::Text(text)=>{
                if let Ok(data)=serde_json::from_str::<Value>(&text){
                    if data["type"] == " register"{
                        if let Some(id)=data["connectionsId"].as_str(){
